...
# 读取csv文件
input1 = pd.read_csv('./clean/考试_clean.csv')
input2 = pd.read_csv('./clean/作弊_clean.csv')
...
contents = pd.concat([input1['content'],input2['content']]).apply(clean_text)
...
# 建立LDA模型
lda = models.ldamodel.LdaModel(corpus=corpus, id2word=dictionary, num_topics=3, passes=100)
...
# 输出主题
for topic in lda.print_topics(num_topics=3, num_words=30):
    print(topic)

data = pyLDAvis.gensim_models.prepare(lda, corpus, dictionary)

pyLDAvis.save_html(data,"lda_html")
